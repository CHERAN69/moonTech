  ...
  return (
    <aside className="w-60 flex-shrink-0 flex flex-col h-screen sticky top-0"
      ...
      <div className="flex items-center gap-2.5 px-5 py-5" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <span className="font-bold text-base" style={{ color: 'var(--text-primary)' }}>ClosePilot</span>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {navItems.map(item => {
          ...
          return (
            ...
            >
              <span style={{ color: active ? '#fff' : 'var(--text-tertiary)' }}>{item.icon}</span>
              {item.label}
            </Link>
          )
        })}
      ...
      <div className="px-3 pb-4 space-y-3">
        <div className="rounded-xl p-4" style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border)' }}>
          <div className="text-xs font-semibold mb-1" style={{ color: 'var(--brand)' }}>Starter Plan</div>
          <div className="text-xs mb-3" style={{ color: 'var(--text-tertiary)' }}>876/1,000 transactions used</div>
          <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--bg-primary)' }}>
            <div className="h-full rounded-full" style={{ width: '87%', background: 'var(--brand)' }}></div>
          </div>
          <Link href="/settings" className="mt-3 block text-center text-xs font-medium py-1.5 rounded-lg text-white transition-opacity hover:opacity-90" style={{ background: 'var(--brand)' }}>
            Upgrade to Growth
          </Link>
        </div>
      </div>
