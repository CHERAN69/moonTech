...
export async function explainAnomalies(
  ...
  // Batch in groups of 10 to save tokens
  ...
  for (const batch of batches) {
    const transactionList = batch.map((p, i) => {
      ...
      return `${i + 1}. ID: ${p.id} | Status: ${p.status} | Amount: ${formatCurrency(tx.amount)} | Date: ${tx.date} | Description: "${tx.description}" | Vendor: "${tx.vendor || 'N/A'}" | Flags: ${p.flags.map(f => f.message).join('; ') || 'none'} | Matched to: ${p.invoice_transaction ? `Invoice ${formatCurrency(p.invoice_transaction.amount)} on ${p.invoice_transaction.date}` : 'nothing'}`
    }).join('\n')

    const prompt = `You are ClosePilot AI, a senior financial analyst assistant for ${companyName}. Analyze these transactions and for each one provide:
...
Transaction: ${formatCurrency(tx.amount)} to "${tx.vendor || tx.description}" on ${tx.date}
...
export async function generateCFOBriefing(
  ...
  const prompt = `You are ClosePilot AI, acting as a fractional CFO advisor for ${companyName}. Generate a concise daily financial briefing based on these metrics.
...
Transaction: ${formatCurrency(tx.amount)} paid to "${tx.vendor || tx.description}" on ${tx.date}
...
Return JSON:
{
  ...
  try {
    ...
    return {
      description: `Payment to ${tx.vendor || tx.description}`,
      date: tx.date,
      lines: [
...
export async function explainSingleAnomaly(pair: {
  ...
  const context = [
    `Status: ${pair.status}`,
    `Bank: $${tx.amount} to "${tx.vendor || tx.description}" on ${tx.date}`,
    inv ? `Invoice: $${inv.amount} from "${inv.vendor || inv.description}" on ${inv.date}${inv.reference ? ` (${inv.reference})` : ''}` : 'Invoice: none',
...
export async function answerFinanceQuery(
  ...
  const prompt = `You are ClosePilot AI, the finance assistant for ${companyName}. Answer this question about their financial data concisely and accurately.
