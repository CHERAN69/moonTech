...
export default function NewReconciliationPage() {
  ...
  const [name, setName]       = useState(`${new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' })} Bank Reconciliation`)
  ...
  const handleFile = async (file: File, type: 'bank' | 'invoice') => {
    ...
    if (type === 'bank') { setBankFile(file); setBankResult(result) }
    else                 { setInvoiceFile(file); setInvoiceResult(result) }
  }
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      fd.append('name', name.trim() || 'Reconciliation')

      setProgress(30)
      const res = await fetch('/api/reconcile', { method: 'POST', body: fd })
      setProgress(80)

      let json: { error?: string; session_id?: string } = {}
      try {
        json = await res.json()
      } catch {
      ...
      if (!res.ok) {
        setError(json.error || 'Reconciliation failed. Please try again.')
        setIsRunning(false)
        setStep(2)
        return
      }
      ...
      router.push(`/reconcile/${json.session_id}`)
    } catch {
              ...
              <div className="mt-4 p-4 rounded-xl border" style={{ background: '#EFF6FF', borderColor: '#BFDBFE' }}>
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm" style={{ color: '#1D4ED8' }}>
                    <strong>FinOpsAi will:</strong> Match transactions using 3-pass algorithm → Flag anomalies with plain-English explanations → Suggest GL categories → Compute your Close Confidence Score
                  </div>
                </div>
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            ...
            <div className="text-sm text-gray-400">
              {progress < 20 ? 'Parsing transactions...' :
               progress < 50 ? 'Running matching engine...' :
               progress < 85 ? 'AI analyzing anomalies...' :
               'Saving results...'}
            </div>
          </div>
        )}
