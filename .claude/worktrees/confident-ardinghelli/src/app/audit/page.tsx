...
function AuditRow({ entry }: { entry: AuditLogEntry }) {
  ...
  const actorLabel = entry.user_email
    ...
    <div className="border-b border-gray-50 last:border-0">
      ...
      >
        ...
        <div className="flex-shrink-0 w-28 truncate">
          <span className="text-xs text-gray-600 font-medium truncate block" title={entry.user_email}>
            {actorLabel}
          </span>
          {entry.user_email && (
            <span className="text-gray-400 text-xs truncate block">{entry.user_email.split('@')[1]}</span>
          )}
        </div>
        ...
        {/* Timestamp */}
        <div className="flex-shrink-0 text-xs text-gray-400 w-20 text-right" title={new Date(entry.created_at).toLocaleString()}>
      ...
      {expanded && (
        <div className="px-5 pb-4 ml-20">
          <div className="rounded-xl border border-gray-100 overflow-hidden text-xs">
            ...
            <div className="px-3 py-2 bg-gray-50 border-t border-gray-100 text-gray-400 flex items-center justify-between">
...
export default function AuditTrailPage() {
  ...
  const buildParams = useCallback((o: number) => {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    setOffset(0)
    try {
      const res = await fetch(`/api/audit?${buildParams(0)}`)
  ...
  const loadMore = async () => {
    ...
    setMore(true)
    try {
      const res = await fetch(`/api/audit?${buildParams(nextOffset)}`)
      if (!res.ok) throw new Error()
      const json = await res.json()
  ...
  const hasMore = !authError && entries.length < total
      ...
      <div className="flex-1 p-6 space-y-5">
        {authError && (
          <div className="rounded-xl px-4 py-3 text-sm flex items-center gap-2 bg-amber-50 text-amber-700">
            ...
            Sign in to view your audit trail.
          </div>
        )}
          ...
          {hasMore && !loading && (
            <div className="px-5 py-4 border-t border-gray-100 text-center">
              <button
                ...
                className="px-5 py-2 rounded-lg text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
              >
                {loadingMore ? (
