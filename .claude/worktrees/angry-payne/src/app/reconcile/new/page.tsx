...
export default function NewReconciliationPage() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      // Re-encode transactions as CSV blob for the API
      const bankBlob = new Blob(
        [bankResult.transactions.map(t => `${t.date},${t.amount},${t.description},${t.vendor ?? ''},${t.reference ?? ''}`).join('\n')],
        { type: 'text/csv' }
      )
      formData.append('bank', bankBlob, 'bank.csv')
      if (invoiceResult) {
        const invBlob = new Blob(
          [invoiceResult.transactions.map(t => `${t.date},${t.amount},${t.description},${t.vendor ?? ''},${t.reference ?? ''}`).join('\n')],
          { type: 'text/csv' }
        )
        formData.append('invoice', invBlob, 'invoice.csv')
      }
      ...
      if (res.ok) {
        ...
        router.push(`/reconcile/${session_id}`)
      } else {
        // Fallback: run engine client-side and go to list
      ...
      // Fallback to list page — session was not persisted
      router.push('/reconcile')
    }
  }
