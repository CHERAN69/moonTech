...
function getPasswordStrength(password: string): {
  ...
  return {
    ...
    bars:   [clamped >= 1, clamped >= 2, clamped >= 3, clamped >= 4],
  }
}
    ...
    <div className="mt-2 space-y-1.5">
      ...
      <div className="flex gap-1">
        {bars.map((filled, i) => (
          <div
            key={i}
            className={cn('h-1 flex-1 rounded-full transition-all duration-300', filled ? '' : 'bg-gray-200')}
      ...
      <div className="flex items-center justify-between">
        ...
        {score < 3 && (
          <span className="text-xs text-gray-400">
            {score < 2 ? 'Add uppercase, numbers & symbols' : 'Add symbols or increase length'}
          </span>
        )}
      </div>
    </div>
