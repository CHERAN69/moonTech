...
/**
 * Universal approval workflow state applied to match pairs,
 * journal entries, close tasks, and reports.
 *
 * Mapping from existing statuses:
 *   JournalEntryStatus draft              → ai_draft
 *   JournalEntryStatus pending_approval   → needs_review
 *   JournalEntryStatus approved           → approved
 *   JournalEntryStatus posted             → locked
 */
...
export type UploadStatus = 'processing' | 'classified' | 'confirmed' | 'error'
...
export interface Upload {
  ...
  detected_entity?: string             // e.g. "Chase Bank", "QuickBooks"
  suggested_period_start?: string
  suggested_period_end?: string
  column_mapping?: Record<string, string>
  transactions_count?: number
  status: UploadStatus
  category_hint?: string               // user-provided hint
  parsed_data?: RawTransaction[]
  session_id?: string
  created_at: string
  updated_at: string
}
